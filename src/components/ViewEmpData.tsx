  ...
  <div className="w-1/4 flex justify-center items-start">
    {allEmp?.profilePhoto ? (
      ...
      />
          ...
          <h2 className="font-bold text-lg mb-2">Education Information</h2>
            ...
            {educMenu.map(({ label, key }) => {
                    ...
                    {value.map((item: any, index: number) => (
                      <div key={index}>
                        <div>
                          ...
                          {item.course},
                          <span className="font-semibold pl-2">Academic:</span>{" "}
                          {item.academic}
                        </div>
                      </div>
                    ))}
              ...
              return (
                <div key={key} className="flex">
                  <p className="px-2 py-1 font-semibold flex-1">{label}</p>
                  <p className="px-2 py-1 font-semibold ">:</p>
                  <p className="px-2 py-1 flex-1">{displayValue}</p>
                </div>
              );
            })}
          </div>

          <h2 className="font-bold text-lg mb-2">Experience Information</h2>

          <div className="flex">
            <p className="flex-1 px-2 py-1 font-semibold align-top">
              Experiences
            </p>
            <p className="px-2 py-1 font-semibold ">:</p>
            <p className="flex-1 px-2 py-1">
          ...
          <h2 className="font-bold text-lg mb-2">Family Information</h2>

          <div>
            {familyMenu.map(({ label, key }) => (
              <div key={key} className="flex">
                <p className="flex-1 px-2 py-1 font-semibold">{label}</p>
                <p className="px-2 py-1 font-semibold ">:</p>
                <p className="flex-1 px-2 py-1">{allEmp[key] || "N/A"}</p>
              </div>
            ))}
          </div>
        </div>
