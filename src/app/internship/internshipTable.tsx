...
import { useIntern } from "../utils/InternContext"; // Adjust the path to where InternContext is located.


export type RequestInternData = {
  intID?: string;
  firstName: string;
  lastName?: string;
  role?: string;
...
export interface InternTableProps {
...
const InternshipTable: React.FC<InternTableProps> = ({ data }) => {
  ...
  const { setEditIntern } = useIntern(); // From context
  ...
  const handleEdit = (intern: RequestInternData) => {
    setEditIntern(intern); // Set the intern to edit in context
    router.push("/internship/addInternship"); // Navigate to the add/edit page
  };
          ...
          <tbody>
            {data.map((intern, index) => (
              <tr key={index} className="text-center text-sm border-b border-[#D2D2D240]">
                ...
                <td className="py-5">
                  <button onClick={() => handleView(intern)} className="text-blue-600 underline">
                    View
                  </button>
                </td>
                <td className="py-5">
                  <button onClick={() => handleEdit(intern)} className="text-blue-600 text-lg">
                    <FiEdit />
                  </button>
                </td>
              </tr>
